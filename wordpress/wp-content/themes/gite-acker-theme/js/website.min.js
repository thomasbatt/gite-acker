...
return target.className=target.className.replace(this.config.animateClass,"").trim()}};WOW.prototype.customStyle=function(box,hidden,duration,delay,iteration){if(hidden){this.cacheAnimationName(box)}box.style.visibility=hidden?"hidden":"visible";if(duration){this.vendorSet(box.style,{animationDuration:duration})}if(delay){this.vendorSet(box.style,{animationDelay:delay})}if(iteration){this.vendorSet(box.style,{animationIterationCount:iteration})}this.vendorSet(box.style,{animationName:hidden?"none":this.cachedAnimationName(box)});return box};WOW.prototype.vendors=["moz","webkit"];WOW.prototype.vendorSet=function(elem,properties){var name,results,value,vendor;results=[];for(name in properties){value=properties[name];elem[""+name]=value;results.push(function(){var j,len,ref,results1;ref=this.vendors;results1=[];for(j=0,len=ref.length;j<len;j++){vendor=ref[j];results1.push(elem[""+vendor+name.charAt(0).toUpperCase()+name.substr(1)]=value)}return results1}.call(this))}return results};WOW.prototype.vendorCSS=function(elem,property){var j,len,ref,result,style,vendor;style=getComputedStyle(elem);result=style.getPropertyCSSValue(property);ref=this.vendors;for(j=0,len=ref.length;j<len;j++){vendor=ref[j];result=result||style.getPropertyCSSValue("-"+vendor+"-"+property)}return result};WOW.prototype.animationName=function(box){var animationName;try{animationName=this.vendorCSS(box,"animation-name").cssText}catch(_error){animationName=getComputedStyle(box).getPropertyValue("animation-name")}if(animationName==="none"){return""}else{return animationName}};WOW.prototype.cacheAnimationName=function(box){return this.animationNameCache.set(box,this.animationName(box))};WOW.prototype.cachedAnimationName=function(box){return this.animationNameCache.get(box)};WOW.prototype.scrollHandler=function(){return this.scrolled=true};WOW.prototype.scrollCallback=function(){var box;if(this.scrolled){this.scrolled=false;this.boxes=function(){var j,len,ref,results;ref=this.boxes;results=[];for(j=0,len=ref.length;j<len;j++){box=ref[j];if(!box){continue}if(this.isVisible(box)){this.show(box);continue}results.push(box)}return results}.call(this);if(!(this.boxes.length||this.config.live)){return this.stop()}}};WOW.prototype.offsetTop=function(element){var top;while(element.offsetTop===void 0){element=element.parentNode}top=element.offsetTop;while(element=element.offsetParent){top+=element.offsetTop}return top};WOW.prototype.isVisible=function(box){var bottom,offset,top,viewBottom,viewTop;offset=box.getAttribute("data-wow-offset")||this.config.offset;viewTop=window.pageYOffset;viewBottom=viewTop+Math.min(this.element.clientHeight,this.util().innerHeight())-offset;top=this.offsetTop(box);bottom=top+box.clientHeight;return top<=viewBottom&&bottom>=viewTop};WOW.prototype.util=function(){return this._util!=null?this._util:this._util=new Util};WOW.prototype.disabled=function(){return!this.config.mobile&&this.util().isMobile(navigator.userAgent)};return WOW}()}).call(this);var ticking=false;var isFirefox=/Firefox/i.test(navigator.userAgent);var isIe=/MSIE/i.test(navigator.userAgent)||/Trident.*rv\:11\./i.test(navigator.userAgent);var scrollSensitivitySetting=30;var slideDurationSetting=600;var currentSlideNumber=0;var totalSlideNumber=$(".background").length;$.fn.detectScroll=function(func){return this.each(function(){var mousewheelEvent=isFirefox?"DOMMouseScroll":"wheel";if(typeof func=="function")this.addEventListener(mousewheelEvent,func,false)})};$.fn.detectSwipe=function(func){return this.each(function(){swipe_det=new Object;swipe_det.sX=0;swipe_det.sY=0;swipe_det.eX=0;swipe_det.eY=0;var min_x=20;var max_x=40;var min_y=40;var max_y=50;var direc="";this.addEventListener("touchstart",function(e){var t=e.touches[0];swipe_det.sX=t.screenX;swipe_det.sY=t.screenY},false);this.addEventListener("touchmove",function(e){e.preventDefault();var t=e.touches[0];swipe_det.eX=t.screenX;swipe_det.eY=t.screenY},false);this.addEventListener("touchend",function(e){if((swipe_det.eX-min_x>swipe_det.sX||swipe_det.eX+min_x<swipe_det.sX)&&(swipe_det.eY<swipe_det.sY+max_y&&swipe_det.sY>swipe_det.eY-max_y)){if(swipe_det.eX>swipe_det.sX)direc="r";else direc="l"}if((swipe_det.eY-min_y>swipe_det.sY||swipe_det.eY+min_y<swipe_det.sY)&&(swipe_det.eX<swipe_det.sX+max_x&&swipe_det.sX>swipe_det.eX-max_x)){if(swipe_det.eY>swipe_det.sY)direc="d";else direc="u"}if(direc!=""){if(typeof func=="function")func(direc)}direc=""},false)})};$.fn.noVisibleFade=function(idSlide){return this.each(function(){var $currentSlide=$(".background").eq(currentSlideNumber);if($currentSlide.attr("id")==idSlide)$(this).removeClass("visible transition-slow").addClass("no-visible transition-fast");else $(this).removeClass("no-visible transition-fast").addClass("visible transition-slow")})};function nextItem(){var $previousSlide=$(".background").eq(currentSlideNumber-1);$previousSlide.removeClass("up-scroll no-display").addClass("down-scroll");var $currentSlide=$(".background").eq(currentSlideNumber);$currentSlide.removeClass("no-visible")}function previousItem(){var $currentSlide=$(".background").eq(currentSlideNumber);$currentSlide.removeClass("down-scroll").addClass("up-scroll")}function scaleCurrentSlideMenu(){$("a.js-scrollTo").removeClass("scale");var selectorSlider='a[data-target="'+currentSlideNumber+'"].js-scrollTo';$(selectorSlider).addClass("scale")}function slideDurationTimeout(slideDuration){setTimeout(function(){ticking=false},slideDuration)}$(window).on("load",function(){var $body="header, #page, footer";var tailleEcranX=window.screen.width;var tailleEcranY=window.screen.height;if(tailleEcranY>530){$($body).animate({opacity:1},1e3,"easeInExpo");$("a").not('a[data-fade="false"]').click(function(e){e.preventDefault();var $urlToLoad=this.href;$($body).animate({opacity:0},1e3,"easeInExpo",function(){window.location=$urlToLoad})})}else{$($body).css({opacity:1})}});$("document").ready(function(){var $body="header, #page, footer";var tailleEcranX=window.screen.width;var tailleEcranY=window.screen.height;if(tailleEcranY>530){function parallaxMove(direction){if(ticking!=true){ticking=true;if(direction==="down"&&currentSlideNumber!==totalSlideNumber-1){currentSlideNumber++;nextItem()}else if(direction==="up"&&currentSlideNumber!==0){currentSlideNumber--;previousItem()}slideDurationTimeout(slideDurationSetting)}scaleCurrentSlideMenu();$(".footer-scrolldown-arrow").noVisibleFade("contact");$("a[data-target=0]").noVisibleFade("accueil")}function parallaxScroll(evt){if(isFirefox){delta=evt.detail*-120}else if(isIe){delta=-evt.deltaY}else{delta=evt.wheelDelta}if(delta<=-scrollSensitivitySetting)var direction="down";else if(delta>=scrollSensitivitySetting)var direction="up";parallaxMove(direction)}function parallaxTouch(d){if(d=="u")direction="down";else if(d=="d")direction="up";parallaxMove(direction)}$("#body").detectScroll(parallaxScroll);$("#page").detectSwipe(parallaxTouch);function loopParallaxMove(direction,n){var slideDuration=0;while(n!=0){setTimeout(function(){ticking=false;parallaxMove(direction)},slideDuration);slideDuration+=slideDurationSetting/2;if(n>0&&direction==="down")n--;else if(n<0&&direction==="up")n++}}function parallaxTarget(targetSlideNumber){var n=targetSlideNumber-currentSlideNumber;if(n>0)loopParallaxMove("down",n);else if(n<0)loopParallaxMove("up",n)}$(".js-scrollTo").on("click",function(evt){var targetSlideNumber=$(this).attr("data-target");if(targetSlideNumber=="next")targetSlideNumber=currentSlideNumber+1;parallaxTarget(targetSlideNumber)});(new WOW).init();$(".js-open-legales").click(function(e){e.preventDefault();$(".legales").fadeIn()});$(".modal").click(function(e){e.preventDefault();$(".legales").fadeOut()})}else{}});